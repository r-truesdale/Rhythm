...
public class SpawnManager : MonoBehaviour
{
    ...
    private bool arrowVisibilityOff = false;
    private void Awake()
    {
        if (Instance == null)
        {
    ...
    {
        arrowVisibilityOff = true;
    }
    public void setArrowVisibilityOn()
    {
        arrowVisibilityOff = false;
    }
    public GameObject SpawnArrow(GameObject arrowPrefab, float arrowSpawnTime, float beatTime, float arrowSpeed)
    {
        ...
        if (PlayerPrefs.GetString("gameState") == "game")//arrows spawning randomly in positions
        {
        ...
        if (arrowVisibilityOff)
        {
            Transform sphere = newArrow.transform.Find("Sphere"); // Assuming the sphere object is a child of the arrow prefab
            if (sphere != null)
            {
