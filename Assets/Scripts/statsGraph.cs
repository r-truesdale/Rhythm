...
public class statsGraph : MonoBehaviour
{
 ...
 private RectTransform lineY;

 private void Start()
 {
  lineY = graphContainer.Find("lineY").GetComponent<RectTransform>();
  PlotGraph();
 }
 private void PlotGraph()
 {
  ...
  for (int j = 0; j <= lineCount; j++)
  {
   ...
   yMax1.rectTransform.anchoredPosition = new Vector2((-graphContainer.rect.width / 2f)-15f, yGraphPosition - (graphContainer.rect.height / 2f));

   yMax1.text = Mathf.Round(yPosition).ToString(); // Set the text of the label
